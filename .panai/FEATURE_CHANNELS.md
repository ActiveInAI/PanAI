...
| -------------- | ------------------------------------ |
| 配对码认证     | 6位随机码，10分钟有效                |
| 本地批准       | 必须在 PanAI 中批准，非 Telegram 中 |
| 用户白名单     | 仅授权用户可使用                     |
| 速率限制       | 防止滥用                             |
...
├─────────────────────────────────────────────────────────────┤
│ Step 4: 配置凭证                                            │
│   PanAI 设置页面 → 粘贴 App ID、App Secret → 验证 → 保存   │
├─────────────────────────────────────────────────────────────┤
│ Step 5: 启动 Bot                                            │
...
与 Telegram 相同，采用本地批准模式。配对码通过 Lark 消息发送给用户，用户在 PanAI 中批准。
