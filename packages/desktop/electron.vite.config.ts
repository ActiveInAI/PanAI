...
// Read the real PanAI version from the repo-root package.json.
  ...
  return {
    main: {
      plugins: [
        ...
        // '@panai/web-host' excluded so its TS sources (which use ESM ".js" import specifiers)
        // are bundled by esbuild rather than left as `require('@panai/web-host')`, which Node
        // cannot resolve because the package ships no compiled .js files (workspace-only).
        externalizeDepsPlugin({ exclude: ['fix-path', '@panai/web-host'] }),
        ...(isDevelopment
          ? [
              {
    ...
    renderer: {
      ...
      appType: 'mpa',
      server: {
        // Default to 5173; when occupied (e.g. another PanAI clone is running),
      ...
      define: {
        ...
        'process.env.SENTRY_DSN': JSON.stringify(process.env.SENTRY_DSN ?? ''),
        // Inject the real PanAI version (root package.json) so renderer code
