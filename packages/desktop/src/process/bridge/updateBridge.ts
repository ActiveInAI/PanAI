...
const DEFAULT_REPO = 'ActiveInAI/PanAI';
const DEFAULT_USER_AGENT = 'PanAI';
const ALLOWED_ASSET_EXTS = new Set(['.exe', '.msi', '.dmg', '.zip', '.deb', '.rpm']);
const CDN_HOST = 'static.panai.local';
...
const resolveRepo = (requestRepo?: string): string => {
  const envRepo = (process.env.PANAI_GITHUB_REPO ?? process.env.AIONUI_GITHUB_REPO)?.trim();
...
const sanitizeFileName = (name: string): string => {
  ...
  // Avoid empty names.
  return base || `PanAI-update-${Date.now()}`;
};
