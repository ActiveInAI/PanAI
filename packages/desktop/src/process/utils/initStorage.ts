...
const STORAGE_PATH = {
  config: 'panai-config.txt',
  chatMessage: 'panai-chat-message.txt',
  chat: 'panai-chat.txt',
  env: '.panai-env',
...
const migrateLegacyData = async () => {
  ...
  try {
    ...
    const isNewDirEmpty =
      ...
      (() => {
        try {
          ...
          console.warn('[PanAI] Warning: Could not read new directory during migration check:', error);
      ...
      if (isVerified) {
        ...
        if (path.resolve(oldDir) !== path.resolve(newDir)) {
          try {
            ...
            console.warn('[PanAI] 原目录清理失败，请手动删除:', oldDir, cleanupError);
          }
        }
      }
    ...
    console.error('[PanAI] 数据迁移失败:', error);
  }
...
const dirConfig = envFile.getSync('panai.dir');
...
const buildMessageListStorage = (conversation_id: string, dir: string) => {
  const fullName = path.join(dir, 'panai-chat-history', conversation_id + '.txt');
  if (!existsSync(fullName)) {
    mkdirSync(path.join(dir, 'panai-chat-history'));
  }
  return JsonFileBuilder<TMessage[]>(path.join(dir, 'panai-chat-history', conversation_id + '.txt'));
};
  ...
  return {
    ...
    backup(conversation_id: string) {
      ...
      return storage.backup(
        path.join(dir, 'panai-chat-history', 'backup', conversation_id + '_' + Date.now() + '.txt')
      );
    },
  };
};
...
const cleanupLegacyBuiltinSkillsDir = () => {
  ...
  fs.rm(legacyDir, { recursive: true, force: true })
    .then(() => console.log('[PanAI] Cleaned up legacy builtin-skills cache'))
    .catch(() => {
 ...
 * `backend builtin assistants catalog` and
 * `backend builtin skills catalog`) — neither is synced from
...
const initStorage = async () => {
  ...
  const mark = (label: string) => console.log(`[PanAI:init] ${label} +${Math.round(performance.now() - t0)}ms`);
  mark('start');
  ...
  //    now live in the backend SQLite catalog and
  ...
  try {
    ...
    console.error('[PanAI] Failed to ensure assistant dirs:', error);
  }
