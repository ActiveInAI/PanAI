...
export const getTempPath = () => {
  ...
  return path.join(rootPath, 'panai');
};
 ...
 * Release builds use ~/.panai; dev builds use ~/.panai-dev.
 * 获取数据目录路径，macOS 上使用符号链接。
 * Release 使用 ~/.panai，Dev 模式使用 ~/.panai-dev。
 */
export const getDataPath = (): string => {
  const rootPath = getElectronPathOrFallback('userData');
  const dataPath = path.join(rootPath, 'panai');
  return ensureCliSafeSymlink(dataPath, getEnvAwareName('.panai'));
};

/**
 * Get config path, using CLI-safe symlink on macOS.
 * Release builds use ~/.panai-config; dev builds use ~/.panai-config-dev.
 * 获取配置目录路径，macOS 上使用符号链接。
 * Release 使用 ~/.panai-config，Dev 模式使用 ~/.panai-config-dev。
 */
export const getConfigPath = (): string => {
  const rootPath = getElectronPathOrFallback('userData');
  const configPath = path.join(rootPath, 'config');
  return ensureCliSafeSymlink(configPath, getEnvAwareName('.panai-config'));
};
...
export async function verifyDirectoryFiles(dir1: string, dir2: string): Promise<boolean> {
  try {
    ...
    console.warn('[PanAI] Error verifying directory files:', error);
    return false;
  }
}
...
export const copyFilesToDirectory = async (
  ...
  for (const file of files) {
    ...
    try {
      ...
      console.warn(`[PanAI] Source file does not exist, skipping: ${absoluteFilePath}`);
      console.warn(`[PanAI] Original path: ${file}`);
      // 跳过不存在的文件，而不是抛出错误
      continue;
    }
    ...
    try {
      ...
      console.error(`[PanAI] Failed to copy file from ${absoluteFilePath} to ${destPath}:`, error);
      // 继续处理其他文件，而不是完全失败
    }
