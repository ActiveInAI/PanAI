...
import { startWebHost, type WebHostHandle } from '@panai/web-host';
...
export const resolveWebUIPort = (
  ...
  const envPort = parsePortValue(process.env.PANAI_PORT ?? process.env.PORT);
...
export async function startDesktopWebUI(opts: { port?: number; allowRemote?: boolean }): Promise<DesktopWebUIHandle> {
  ...
  const handle = await startWebHost({
    app: {
      ...
      // getDataPath() returns ~/.panai[-dev] symlink on macOS to sidestep
