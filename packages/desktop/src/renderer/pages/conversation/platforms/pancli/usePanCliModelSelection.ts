...
export type PanCliModelSelection = {
...
export type UsePanCliModelSelectionOptions = {
  initialModel: TProviderWithModel | undefined;
  onSelectModel: (provider: IProvider, modelName: string) => Promise<boolean>;
};

export const usePanCliModelSelection = ({
  initialModel,
  onSelectModel,
}: UsePanCliModelSelectionOptions): PanCliModelSelection => {
  ...
  // PanAI backend does not support Google Auth — filter it out
