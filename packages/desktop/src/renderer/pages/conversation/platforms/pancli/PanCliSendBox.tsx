...
import { usePanCliMessage } from './usePanCliMessage';
import type { PanCliModelSelection } from './usePanCliModelSelection';

const usePanCliSendBoxDraft = getSendBoxDraftHook('aionrs', {
  _type: 'aionrs',
  atPath: [],
  content: '',
  uploadFile: [],
...
const useSendBoxDraft = (conversation_id: string) => {
  const { data, mutate } = usePanCliSendBoxDraft(conversation_id);
...
const PanCliSendBox: React.FC<{
  conversation_id: string;
  modelSelection: PanCliModelSelection;
  session_mode?: string;
  agent_name?: string;
  ...
  const { thought, running, hasHydratedRunningState, setActiveMsgId, setWaitingResponse, resetState } =
    usePanCliMessage(conversation_id, {
  ...
  useEffect(() => {
    ...
    const processInitialMessage = async () => {
      ...
      try {
        ...
        console.error('[PanCliSendBox] Failed to send initial message:', error);
  ...
  const handleSheetModeChange = useCallback(
    async (mode: string) => {
      ...
      try {
        ...
        console.error('[PanCliSendBox] Failed to switch mode via sheet:', error);
  ...
  const handleStop = async (): Promise<void> => {
    ...
    try {
      ...
      console.warn('[PanCliSendBox] stop request failed', error);
    } finally {
      resetState();
            ...
            {uploadFile.length > 0 && (
              <HorizontalFileList>
                {uploadFile.map((path) => (
                  <FilePreview
                    key={path}
                    data-testid={`pancli-file-tag-${uploadFile.indexOf(path)}`}
                    path={path}
                ...
                {atPath.map((item) => {
                  ...
                  if (!item.isFile) {
                    ...
                    return (
                      <Tag
                        key={item.path}
                        data-testid={`pancli-folder-tag-${folderIndex}`}
                        color='blue'
                        closable
                        onClose={() => {
