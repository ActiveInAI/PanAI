...
/** PanModal 组件 Props */
export interface PanModalProps extends Omit<ModalProps, 'title' | 'footer'> {
 ...
 * ```tsx
 * // 基本用法 / Basic usage
 * <PanModal visible={true} onCancel={handleClose} header="标题">
 *   内容
 * </PanModal>
 *
 * // 预设尺寸 / Preset size
 * <PanModal visible={true} size="large" header="大型弹窗">
 *   内容
 * </PanModal>
 *
...
const PanModal: React.FC<PanModalProps> = ({
  children,
  size,
  header,
  footer,
  ...
  return (
    <Modal
      ...
      footer={null}
      onCancel={onCancel}
      className={`panai-modal ${className}`}
      style={finalStyle}
      getPopupContainer={() => document.body}
    >
      <div className='panai-modal-wrapper' style={{ borderRadius: borderRadiusVal }}>
        {renderHeader()}
        <div className='panai-modal-body-content' style={bodyInlineStyle}>
          {children}
        </div>
        {renderFooter()}
      </div>
