 ...
 */
export interface PanCollapseProps {
 ...
 */
export interface PanCollapseItemProps {
 ...
 */
const PanCollapseItem: React.FC<PanCollapseItemProps> = ({ children }) => <>{children}</>;
PanCollapseItem.displayName = 'PanCollapseItem';
 ...
 * ```tsx
 * // 基本用法 / Basic usage
 * <PanCollapse defaultActiveKey={['1']}>
 *   <PanCollapse.Item name="1" header="面板1">
 *     内容1
 *   </PanCollapse.Item>
 *   <PanCollapse.Item name="2" header="面板2">
 *     内容2
 *   </PanCollapse.Item>
 * </PanCollapse>
 *
 * // 手风琴模式 / Accordion mode
 * <PanCollapse accordion defaultActiveKey="1">
 *   <PanCollapse.Item name="1" header="面板1">内容1</PanCollapse.Item>
 *   <PanCollapse.Item name="2" header="面板2">内容2</PanCollapse.Item>
 * </PanCollapse>
 *
 * // 自定义图标 / Custom icon
 * <PanCollapse
 ...
 * ```
 */
const PanCollapseComponent: React.FC<PanCollapseProps> & { Item: typeof PanCollapseItem } = ({
  children,
  className,
  defaultActiveKey,
  activeKey,
  ...
  const items = useMemo(() => {
    return React.Children.toArray(children).filter((child): child is React.ReactElement<PanCollapseItemProps> => {
      return React.isValidElement(child) && child.type === PanCollapseItem;
    });
  }, [children]);
...
PanCollapseComponent.Item = PanCollapseItem;

export default PanCollapseComponent;
