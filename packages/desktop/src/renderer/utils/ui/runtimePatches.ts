...
declare global {
  interface Window {
    __PanAISafeResizeObserver__?: boolean;
    __PanAIResizeObserverPatched__?: boolean;
  }
...
const patchResizeObserver = () => {
  ...
  if (!window.__PanAISafeResizeObserver__ && typeof ResizeObserver !== 'undefined') {
    ...
    window.__PanAISafeResizeObserver__ = true;
  }
};
...
const patchGlobalErrorFilters = () => {
  ...
  if (!window.__PanAIResizeObserverPatched__) {
    ...
    window.__PanAIResizeObserverPatched__ = true;
  }
};

const patchConsole = () => {
  // Console patch mirrors the listener filters so devtools logs stay clean（控制台同样做拦截，防止被重复警告淹没）.
  if (typeof console !== 'undefined' && !console.__PanAIResizeObserverPatched__) {
    ...
    console.__PanAIResizeObserverPatched__ = true;
  }
};
