/**
 * PanCli Chat E2E Tests - Combo Scenarios (P1)
 *
 * Test Cases Covered:
...
test.describe('PanCli Chat - Combo Scenarios (P1)', () => {
  ...
  test('TC-A-10: should handle folder + second model + yolo mode combo', async ({ page }) => {
    ...
    try {
      ...
      await takeScreenshot(page, `chat-pancli/tc-a-10/01-before.png`);
      ...
      await takeScreenshot(page, `chat-pancli/tc-a-10/02-model-switched.png`);

      // Step 5: Send message
      await sendPanCliMessage(page, conversationId, 'List files in the attached folder.');
      await waitForPanCliReply(page, conversationId);

      // Screenshot 03: reply completed
      await takeScreenshot(page, `chat-pancli/tc-a-10/03-reply.png`);
  ...
  test('TC-A-11: should handle file + non-default model + default mode combo', async ({ page }) => {
    ...
    try {
      ...
      // Screenshot 01: before creation
      await takeScreenshot(page, `chat-pancli/tc-a-11/01-before.png`);
      ...
      // Screenshot 02: after model switch
      await takeScreenshot(page, `chat-pancli/tc-a-11/02-model-switched.png`);

      // Step 5: Send message about file
      await sendPanCliMessage(page, conversationId, 'Read the content of test-file.txt in the workspace.');
      await waitForPanCliReply(page, conversationId);

      // Screenshot 03: reply completed
      await takeScreenshot(page, `chat-pancli/tc-a-11/03-reply.png`);
  ...
  test('TC-A-12: should handle full combo (folder + file + second model + yolo)', async ({ page }) => {
    ...
    try {
      ...
      // Screenshot 01: before creation
      await takeScreenshot(page, `chat-pancli/tc-a-12/01-before.png`);
      ...
      // Screenshot 02: after model switch
      await takeScreenshot(page, `chat-pancli/tc-a-12/02-model-switched.png`);

      // Step 5: Send message about folder and files
      await sendPanCliMessage(page, conversationId, 'List all files in the attached folder and read their contents.');
      await waitForPanCliReply(page, conversationId);

      // Screenshot 03: reply completed
      await takeScreenshot(page, `chat-pancli/tc-a-12/03-reply.png`);
