/**
 * PanCli Chat E2E Tests - Permission Modes (P0 + P1)
 *
 * Test Cases Covered:
...
test.describe('PanCli Chat - Permission Modes (P0 + P1)', () => {
  ...
  test('TC-A-05: should use yolo permission selected on guid page', async ({ page }) => {
    ...
    try {
      ...
      await takeScreenshot(page, `chat-pancli/tc-a-05/01-guid-page.png`);

      // Step 2: Select aionrs agent
      await selectPanCliAgent(page);
      ...
      await takeScreenshot(page, `chat-pancli/tc-a-05/02-mode-selector-open.png`);
      ...
      await takeScreenshot(page, `chat-pancli/tc-a-05/03-yolo-selected.png`);
      ...
      await takeScreenshot(page, `chat-pancli/tc-a-05/04-reply-completed.png`);
  ...
  test('TC-A-06: should switch permission mid-conversation and persist to DB', async ({ page }) => {
    ...
    try {
      ...
      await takeScreenshot(page, `chat-pancli/tc-a-06/01-conversation-created.png`);
      ...
      await takeScreenshot(page, `chat-pancli/tc-a-06/02-first-reply.png`);
      ...
      await takeScreenshot(page, `chat-pancli/tc-a-06/03-mode-selector-open.png`);
      ...
      await takeScreenshot(page, `chat-pancli/tc-a-06/04-mode-switched.png`);

      // Step 6: Send second message
      await sendPanCliMessage(page, conversationId, 'What mode are we using now?');

      // Step 7: Wait for second AI reply
      await waitForPanCliReply(page, conversationId);

      // Screenshot 05: second reply completed
      await takeScreenshot(page, `chat-pancli/tc-a-06/05-second-reply.png`);
