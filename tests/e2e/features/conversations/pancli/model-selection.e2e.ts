/**
 * PanCli Chat E2E Tests - Model Selection (P0 + P1)
 *
 * Test Cases Covered:
...
test.describe('PanCli Chat - Model Selection (P0 + P1)', () => {
  ...
  test.skip('TC-A-04: should use second model selected on guid page', async ({ page }) => {
    ...
    try {
      ...
      await takeScreenshot(page, `chat-pancli/tc-a-04/01-guid-page-initial.png`);
      ...
      await takeScreenshot(page, `chat-pancli/tc-a-04/02-conversation-created.png`);

      // Step 3: Send message
      await sendPanCliMessage(page, conversationId, 'Say hi in one word.');
      await takeScreenshot(page, `chat-pancli/tc-a-04/03-message-sent.png`);

      // Step 4: Wait for AI reply
      await waitForPanCliReply(page, conversationId);
      await takeScreenshot(page, `chat-pancli/tc-a-04/04-reply-completed.png`);
  ...
  test.skip('TC-A-07: should switch model mid-conversation and update DB', async ({ page }) => {
    ...
    try {
      ...
      await takeScreenshot(page, `chat-pancli/tc-a-07/01-conversation-created.png`);

      // Step 2: Send first message
      await sendPanCliMessage(page, conversationId, 'Hello, please respond.');

      // Step 3: Wait for first AI reply
      await waitForPanCliReply(page, conversationId);

      // Screenshot 02: first reply completed
      await takeScreenshot(page, `chat-pancli/tc-a-07/02-first-reply.png`);
      ...
      await takeScreenshot(page, `chat-pancli/tc-a-07/03-model-selector-open.png`);
      ...
      await sendPanCliMessage(page, conversationId, 'What model are you using now?');

      // Step 7: Wait for second AI reply
      await waitForPanCliReply(page, conversationId);

      // Screenshot 04: second reply completed
      await takeScreenshot(page, `chat-pancli/tc-a-07/04-second-reply.png`);
