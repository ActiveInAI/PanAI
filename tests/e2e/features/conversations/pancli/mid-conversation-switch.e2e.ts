/**
 * PanCli Chat E2E Tests - Mid-Conversation Switch (P1)
 *
 * Test Cases Covered:
 ...
 * - PanCliModelSelector: data-testid="pancli-model-selector"
 * - AgentModeSelector: data-testid="agent-mode-selector-aionrs"
 */

import { test, expect } from '../../../fixtures';
import {
  resolvePanCliPreconditions,
  cleanupE2EPanCliConversations,
  createPanCliConversationViaBridge,
  sendPanCliMessage,
  waitForPanCliReply,
  getPanCliConversationDB,
  getPanCliMessages,
  createTempWorkspace,
  type PanCliTestModels,
} from '../../../helpers';
import { takeScreenshot } from '../../../helpers/screenshots';

test.describe('PanCli Chat - Mid-Conversation Switch (P1)', () => {
  test.setTimeout(180000); // 3 minutes (longer for multi-round tests)

  let preconditions: { binary: string | null; models: PanCliTestModels | null };

  test.beforeAll(async ({ page }) => {
    preconditions = await resolvePanCliPreconditions(page);
  ...
  test.afterEach(async ({ page }) => {
    ...
    await cleanupE2EPanCliConversations(page);
  ...
  test.skip('TC-A-08: should handle continuous switch (model → permission → model)', async ({ page }) => {
    ...
    // See tests/e2e/docs/chat-pancli/implementation-mapping.zh.md "Known Issues" section
    ...
    try {
      ...
      const conversationId = await createPanCliConversationViaBridge(page, {
      ...
      await takeScreenshot(page, `chat-pancli/tc-a-08/01-created.png`);

      // Step 2: Send initial message
      await sendPanCliMessage(page, conversationId, 'Hello, initial message.');
      await waitForPanCliReply(page, conversationId);

      // Screenshot 02: initial reply
      await takeScreenshot(page, `chat-pancli/tc-a-08/02-initial-reply.png`);
      ...
      const modelSelector = page.locator('[data-testid="pancli-model-selector"]');
      ...
      await takeScreenshot(page, `chat-pancli/tc-a-08/03-model-switched.png`);
      ...
      await takeScreenshot(page, `chat-pancli/tc-a-08/04-permission-switched.png`);
      ...
      await takeScreenshot(page, `chat-pancli/tc-a-08/05-model-switched-again.png`);

      // Step 7: Send message after all switches
      await sendPanCliMessage(page, conversationId, 'After all switches.');
      await waitForPanCliReply(page, conversationId);

      // Screenshot 06: final reply
      await takeScreenshot(page, `chat-pancli/tc-a-08/06-final-reply.png`);
      ...
      const conversation = await getPanCliConversationDB(page, conversationId);
      const extra =
  ...
  test.skip('TC-A-09: should handle 3 rounds of conversation after model/permission switch', async ({ page }) => {
    ...
    try {
      ...
      // Step 2: Send initial message
      await sendPanCliMessage(page, conversationId, 'Round 1: Initial message.');
      await waitForPanCliReply(page, conversationId);

      // Screenshot 01: after round 1
      await takeScreenshot(page, `chat-pancli/tc-a-09/01-round1.png`);
      ...
      await takeScreenshot(page, `chat-pancli/tc-a-09/02-after-switches.png`);

      // Step 6: Round 2 - send and wait
      await sendPanCliMessage(page, conversationId, 'Round 2: After model and permission switch.');
      await waitForPanCliReply(page, conversationId);

      // Screenshot 03: after round 2
      await takeScreenshot(page, `chat-pancli/tc-a-09/03-round2.png`);

      // Step 7: Round 3 - send and wait
      await sendPanCliMessage(page, conversationId, 'Round 3: Continue with switched config.');
      await waitForPanCliReply(page, conversationId);

      // Screenshot 04: after round 3
      await takeScreenshot(page, `chat-pancli/tc-a-09/04-round3.png`);

      // Step 8: Round 4 (bonus) - send and wait
      await sendPanCliMessage(page, conversationId, 'Round 4: Final message.');
      await waitForPanCliReply(page, conversationId);

      // Screenshot 05: after round 4
      await takeScreenshot(page, `chat-pancli/tc-a-09/05-round4.png`);
