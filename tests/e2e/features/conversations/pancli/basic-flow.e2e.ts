/**
 * PanCli Chat E2E Tests - Basic Flow (P0 Priority)
 *
 * Test Cases Covered:
 ...
 *
 * Prerequisites:
 * - aionrs binary available (via ipcBridge.fs.findPanCliBinary)
 * - User logged in
 ...
 * - PanCliSendBox: data-testid="pancli-sendbox"
 * - FileAttachButton: data-testid="pancli-attach-folder-btn"
 */
...
test.describe('PanCli Chat - Basic Flow (P0)', () => {
  // Set longer timeout for aionrs tests (binary calls can be slow)
  test.setTimeout(240_000); // 4 minutes — allow 150s waitForPanCliReply + buffer
  ...
  test('TC-A-01: should complete minimal conversation with no attachments', async ({ page }) => {
    ...
    try {
      ...
      await takeScreenshot(page, `chat-pancli/tc-a-01/01-guid-page-initial.png`);
      ...
      await takeScreenshot(page, `chat-pancli/tc-a-01/02-conversation-created.png`);

      // Step 3: Send simple message
      await sendPanCliMessage(page, conversationId, 'Say hi in one word.');
      await takeScreenshot(page, `chat-pancli/tc-a-01/03-message-sent.png`);

      // Step 4: Wait for AI reply
      await waitForPanCliReply(page, conversationId);
      await takeScreenshot(page, `chat-pancli/tc-a-01/04-reply-completed.png`);
  ...
  test('TC-A-02: should associate single folder and reference in message', async ({ page }) => {
    ...
    try {
      ...
      await takeScreenshot(page, `chat-pancli/tc-a-02/01-guid-page-before-create.png`);
      ...
      await sendPanCliMessage(page, conversationId, 'What files are in the attached folder?');

      // Screenshot 02: message sent
      await takeScreenshot(page, `chat-pancli/tc-a-02/02-message-sent.png`);

      // Step 4: Wait for AI reply
      await waitForPanCliReply(page, conversationId);

      // Screenshot 03: AI reply completed
      await takeScreenshot(page, `chat-pancli/tc-a-02/03-reply-completed.png`);
  ...
  test('TC-A-03: should upload single file and binary receives file parameter', async ({ page }) => {
    ...
    try {
      ...
      await takeScreenshot(page, `chat-pancli/tc-a-03/01-before-create.png`);
      ...
      // Screenshot 02: message sent
      await takeScreenshot(page, `chat-pancli/tc-a-03/02-message-sent.png`);

      // Step 4: Wait for AI reply
      await waitForPanCliReply(page, conversationId);

      // Screenshot 03: reply completed
      await takeScreenshot(page, `chat-pancli/tc-a-03/03-reply-completed.png`);
