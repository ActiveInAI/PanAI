 ...
 * so seeding a legacy `panai-config.txt` before launch and observing
...
function querySqliteIds(dataDir: string, sql: string): string[] {
  const dbPath = path.join(dataDir, 'panai.db');
...
test.describe('Assistant User Data Migration (T5)', () => {
  ...
  test.describe('Migration contract (sibling backend)', () => {
    ...
    test.beforeEach(async () => {
      dataDir = fs.mkdtempSync(path.join(os.tmpdir(), 'panai-e2e-migrate-'));
      await startBackend();
    });
