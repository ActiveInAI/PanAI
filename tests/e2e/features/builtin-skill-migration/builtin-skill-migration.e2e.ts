 ...
 */
const AUTO_INJECT_EXPECTED_NAMES = ['panai-skills', 'cron', 'officecli', 'skill-creator'] as const;
 ...
 */
const AUTO_INJECT_DIR_NAMES = ['panai-skills', 'cron', 'office-cli', 'skill-creator'] as const;
...
function resolveBackendBinary(): string {
  const candidates = [process.env.PANAI_BACKEND_BINARY, path.join(os.homedir(), '.cargo', 'bin', 'aioncore')].filter(
    (x): x is string => typeof x === 'string' && x.length > 0
  );
  for (const c of candidates) {
    if (fs.existsSync(c)) return c;
  }
  throw new Error('aioncore binary not found. Set PANAI_BACKEND_BINARY or install to ~/.cargo/bin/aioncore.');
}
...
test.describe('Built-in Skill Migration (T3)', () => {
  ...
  test('S7: builtin skills in /api/skills expose an absolute, readable location for export', async ({ page }) => {
    ...
    const targetDir = fs.mkdtempSync(path.join(os.tmpdir(), 'panai-e2e-s7-export-'));
    try {
  ...
  test.describe('Cold-start invariants (sibling backend)', () => {
    ...
    async function startBackend(): Promise<void> {
      ...
      delete parentEnv.PANAI_EXTENSIONS_PATH;
      delete parentEnv.PANAI_EXTENSION_STATES_FILE;
      delete parentEnv.PANAI_E2E_TEST;
      delete parentEnv.PANAI_CDP_PORT;
      delete parentEnv.PANAI_BUILTIN_SKILLS_PATH;
    ...
    test.beforeEach(() => {
      dataDir = fs.mkdtempSync(path.join(os.tmpdir(), 'panai-e2e-builtin-skill-'));
    });
    ...
    test('S8: legacy {cacheDir}/builtin-skills/ is gone after the current Electron boot', async () => {
      ...
      // `~/.panai-config` tree. Failing that we at least assert the
      ...
      const candidates = [
        path.join(os.homedir(), '.panai-config', 'builtin-skills'),
        path.join(os.homedir(), '.panai-config-dev', 'builtin-skills'),
      ];
      ...
      test.info().annotations.push({
        ...
        description:
          survivors.length === 0
            ? 'no legacy builtin-skills cache dirs detected under ~/.panai-config*'
