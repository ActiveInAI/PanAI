/**
 * E2E test helpers for aionrs (Pan CLI) conversations.
 */
...
/**
 * PanCli test models structure.
 */
export interface PanCliTestModels {
 ...
 */
export function resolvePanCliBinary(): string | null {
  try {
 ...
 */
export async function getPanCliTestModels(page: Page): Promise<PanCliTestModels | null> {
  try {
    const providers = await invokeBridge<any[]>(page, 'mode.get-model-config', {});
    if (!Array.isArray(providers)) return null;

    const isPanCliCompatible = (p: any): boolean => {
    ...
    const candidates = providers.filter(
      (p) => p.enabled !== false && Array.isArray(p.model) && p.model.length > 0 && p.apiKey && isPanCliCompatible(p)
    );
 ...
 */
export async function resolvePanCliPreconditions(page: Page): Promise<{
  binary: string | null;
  models: PanCliTestModels | null;
}> {
  const binary = resolvePanCliBinary();
  const models = await getPanCliTestModels(page);
 ...
 */
export async function createPanCliConversationViaBridge(
  page: Page,
  opts: {
    name?: string;
    workspace: string;
  ...
  if (!result?.id) {
    throw new Error(
      `createPanCliConversationViaBridge: bridge returned no conversation id — check provider apiKey and platform fields`
    );
  }
  return result.id;
}
 ...
 */
export async function sendPanCliMessage(
  page: Page,
 ...
 */
export async function waitForPanCliReply(page: Page, conversationId: string, timeoutMs = 150_000): Promise<void> {
  const deadline = Date.now() + timeoutMs;
  let lastAiMessageLength = 0;
  let stableSince = 0;

  while (Date.now() < deadline) {
    const messages = await getPanCliMessages(page, conversationId);
  ...
  const finalConv = await getPanCliConversationDB(page, conversationId);
  const finalMsgs = await getPanCliMessages(page, conversationId);
  console.error(`[waitForPanCliReply TIMEOUT] conv.status=${finalConv?.status}, msg count=${finalMsgs.length}`);
  for (const m of finalMsgs) {
    ...
    console.error(
      `[waitForPanCliReply TIMEOUT]   - pos=${m.position} type=${m.type} status=${m.status} preview="${preview}"`
    );
  }
  throw new Error(`PanCli reply timeout after ${timeoutMs}ms for conversation ${conversationId}`);
}
 ...
 */
export async function getPanCliConversationDB(page: Page, conversationId: string): Promise<any> {
  try {
 ...
 */
export async function getPanCliMessages(page: Page, conversationId: string): Promise<any[]> {
  try {
 ...
 * @param page Playwright page
 */
export async function cleanupE2EPanCliConversations(page: Page): Promise<void> {
...
export function createTempWorkspace(scenario: string): { path: string; cleanup: () => void } {
  const timestamp = Date.now();
  const dirPath = `/tmp/e2e-chat-pancli-${scenario}-${timestamp}`;
 ...
 * @param page Playwright page
 */
export async function selectPanCliAgent(page: Page): Promise<void> {
  await goToGuid(page);
 ...
 */
export async function selectPanCliModel(page: Page, modelId: string): Promise<void> {
  const modelBtn = page.locator('[data-testid="pancli-model-selector"]');
 ...
 */
export async function selectPanCliMode(page: Page, mode: string): Promise<void> {
 ...
 */
export async function attachPanCliFolder(page: Page, folderPath: string): Promise<void> {
  ...
  const attachBtn = page.locator('[data-testid="pancli-attach-folder-btn"]');
  await attachBtn.waitFor({ state: 'visible', timeout: 10_000 });

  // Skip actual click for smoke test (would trigger file dialog)
  console.log(`[attachPanCliFolder] Attach button found (folder: ${folderPath})`);
}
 ...
 */
export async function uploadPanCliFiles(page: Page, filePaths: string[]): Promise<void> {
  const fileInput = page.locator('[data-testid="pancli-file-upload-input"]');
  await fileInput.setInputFiles(filePaths);

  // Wait for file preview tags to appear
  await page.waitForSelector('[data-testid^="pancli-file-tag-"]', { timeout: 5_000 });
}
