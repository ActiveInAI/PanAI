...
const emptyExtensionsDir = fs.mkdtempSync(path.join(os.tmpdir(), 'panai-e2e-no-extensions-'));
const stateSandboxDir = fs.mkdtempSync(path.join(os.tmpdir(), 'panai-e2e-no-extensions-state-'));
...
async function launchAppWithoutExtensions(): Promise<ElectronApplication> {
  ...
  return electron.launch({
    ...
    env: {
      ...process.env,
      PANAI_EXTENSIONS_PATH: emptyExtensionsDir,
