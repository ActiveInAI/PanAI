...
```typescript
const e2eStateSandboxDir = fs.mkdtempSync(path.join(os.tmpdir(), 'panai-e2e-state-'));
...
```typescript
PANAI_EXTENSION_STATES_FILE: process.env.PANAI_EXTENSION_STATES_FILE || e2eStateFile;
```
...
**Critical bottleneck**: `workers: 1` enforced + shared `panai.db` → parallel execution impossible without refactoring.
...
1. Extend `PANAI_E2E_TEST` to include worker ID:
   ```typescript
   PANAI_E2E_TEST_WORKER_ID: process.env.PLAYWRIGHT_WORKER_INDEX || '0';
   ```
2. Modify `getDevAppName()` to return worker-specific name:
   ```typescript
   const workerId = process.env.PANAI_E2E_TEST_WORKER_ID || '0';
   return `PanAI-E2E-Worker-${workerId}`;
   ```
3. Each worker gets isolated:
   - `~/Library/Application Support/PanAI-E2E-Worker-0/config/panai.db`
   - `~/Library/Application Support/PanAI-E2E-Worker-1/config/panai.db`
...
**Complexity**: High (requires main process to read `PANAI_DATABASE_PATH`, conflicts with userData convention)
