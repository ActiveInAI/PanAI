# Pan CLI (aionrs) E2E 测试需求

**版本**: v1.1（修订版）
**作者**: chat-pancli-analyst
**日期**: 2026-04-22
...
2. **aionrs 对话页** (`src/renderer/pages/conversation/platforms/pancli/PanCliChat.tsx:27-54`)
   - 显示 `MessageList`（历史消息）
   - `PanCliSendBox` 提供文件上传、文件夹关联、模型选择、权限选择
   - 发送消息后，前端通过 `ipcBridge.conversation.sendMessage.invoke()` 调用进程端

3. **进程端** (`src/process/task/PanCliManager.ts:78-781`)
   - 创建 `PanCliAgent` 实例（`src/process/agent/aionrs/index.ts:54-450`）
...
4. **后端持久化**
   - backend 独占 `panai.db`，Electron 不再直接访问 SQLite
...
| 维度           | 档位 | 说明                                   | 源码追溯                                                 |
| -------------- | ---- | -------------------------------------- | -------------------------------------------------------- |
| **文件夹关联** | 2 档 | 无关联 / 关联                          | `PanCliSendBox.tsx:331-337` atPath 状态 + event listener |
| **文件上传**   | 2 档 | 无上传 / 上传                          | `PanCliSendBox.tsx:103-125` file input handler           |
...
| **对话中切换** | 必测 | 切换模型 + 切换权限                    | `PanCliModelSelector.tsx` + `AgentModeSelector`          |
...
- 发送消息时 `atPath = []`
- `workspace` 指向临时目录 `/tmp/e2e-chat-pancli-<scenario>-<ts>/`

**档位 2 - 关联文件夹**:

- 通过 `emitter.emit('aionrs.selected.file', items)` 触发
- `atPath` 包含 `{path: '/tmp/...', name: 'folder-name', isFile: false}`
- 对话页显示蓝色 Tag（`PanCliSendBox.tsx:423-446`）

**前置条件**:

- E2E 在 `/tmp/e2e-chat-pancli-<scenario>-<ts>/` 创建临时文件夹
...
- E2E 创建测试文件 `/tmp/e2e-chat-pancli-<scenario>-<ts>/test.txt`（内容: "E2E test file"）

**验证点**:

- 上传后显示文件预览卡片（`PanCliSendBox.tsx:413-421`）
...
- `src/renderer/pages/conversation/platforms/pancli/usePanCliModelSelection.ts:34-40`:
  ```typescript
...
- aionrs **不支持 Google Auth**（`usePanCliModelSelection.ts:36-40` 过滤）
...
| mode        | label             | 行为                                             | 源码追溯                                |
| ----------- | ----------------- | ------------------------------------------------ | --------------------------------------- |
| `default`   | Default           | 每次工具调用需确认                               | `PanCliManager.ts:250-296` confirm 逻辑 |
| `auto_edit` | Auto-Accept Edits | 自动批准 edit / info 类工具，exec / mcp 仍需确认 | `PanCliManager.ts:254-259`              |
| `yolo`      | YOLO              | 全自动批准所有工具                               | `PanCliManager.ts:250-253`              |

**权限切换接口**:

- guid 页: `GuidActionRow.tsx:277-287` — `AgentModeSelector` 组件
- 对话页: `PanCliSendBox.tsx:391-401` — 同样使用 `AgentModeSelector`
- 进程端: `PanCliManager.ts:727-737` — `setMode()` 更新 DB + 发送 `set_mode` 到 binary
...
- 操作: 点击对话页 `PanCliModelSelector` 按钮 → 选择不同模型
...
- 预期: 立即生效（`PanCliManager.setMode()` 发送 `set_mode` 到 binary）
...
const binary = await ipcBridge.fs.findPanCliBinary.invoke();
if (!binary) {
...
- 解析顺序: 环境变量 `AION_CLI_PATH` → `~/.panai/bin/aion-<platform>-<arch>` → 系统 PATH 中的 `aion` 命令
...
export async function checkPanCliBinary(page: Page): Promise<boolean> {
  try {
    const binary = await invokeBridge(page, 'fs.findPanCliBinary');
    if (!binary) {
...
// tests/e2e/specs/chat-pancli/*.spec.ts
test.beforeAll(async ({ page }) => {
  const hasBinary = await checkPanCliBinary(page);
  if (!hasBinary) {
...
```
/tmp/e2e-chat-pancli-<scenario>-<timestamp>/
...
| **FS 临时目录**       | `rm -rf /tmp/e2e-chat-pancli-*`         | `fs.existsSync()` 期望 false  |
...
| `type`       | TEXT        | 固定 `'aionrs'`                        | -                          |
| `model`      | TEXT        | 模型 ID（如 `'claude-opus-4-7'`）      | `PanCliManager.ts:108`     |
| `status`     | TEXT        | `'pending' \| 'running' \| 'finished'` | `PanCliManager.ts:524-526` |
...
- `sessionMode`: `PanCliManager.ts:740-747` — `saveSessionMode()`
- `lastTokenUsage`: `PanCliManager.ts:432-449` — `saveContextUsage()`
...
```tsx
// src/renderer/pages/conversation/platforms/pancli/PanCliSendBox.tsx
<div data-testid="pancli-sendbox">
  {/* SendBox 根元素 */}
</div>

// src/renderer/pages/conversation/platforms/pancli/PanCliModelSelector.tsx
<Button data-testid="pancli-model-selector">
  {/* 模型选择器按钮 */}
</Button>
...
// src/renderer/pages/conversation/platforms/pancli/PanCliSendBox.tsx（file input）
<input
  type="file"
...
// 文件夹 Tag
<Tag key={item.path} data-testid={`pancli-folder-tag-${idx}`} />
...
| 进程崩溃        | 前端收到 `error` 事件，对话标记为 error | `PanCliManager.ts:297-298` |
...
**预期**: 每个对话独立维护 binary 进程 + session（`PanCliManager` 实例独立）
...
```typescript
// tests/e2e/setup/aionrs.setup.ts
export async function getPanCliTestModels(page: Page): Promise<{
...
- E2E beforeAll 检查 `getPanCliTestModels()` 返回值
...
**背景**: `PanCliAgent.setConfig(model)` 发送 `set_config` 命令，但 binary 能力未验证
...
**背景**: 当前存储在内存（`PanCliApprovalStore`），进程重启后失效
...
**背景**: 当前代码未显式处理（`PanCliManager.ts:253-296` confirm 逻辑独立）
...
| 文档            | 路径                                              | 状态                        |
| --------------- | ------------------------------------------------- | --------------------------- |
| Gate 1 需求文档 | `tests/e2e/docs/chat-pancli/requirements.zh.md`   | ✅ 完成（本文档）           |
| Gate 1 讨论记录 | `tests/e2e/docs/chat-pancli/discussion-log.zh.md` | ✅ 完成（双 reviewer 审核） |
| Gate 2 测试用例 | `tests/e2e/docs/chat-pancli/test-cases.zh.md`     | ⏳ 待 designer 起草         |
| Gate 3 实现映射 | `tests/e2e/docs/chat-pancli/implementation.md`    | ⏳ 待 engineer 起草         |
...
2. **§8 议题 0（新增）** — E2E 环境 provider 配置前置条件
   ...
   - 动态 model 选择实现: `getPanCliTestModels()` helper

3. **源码追溯补充**:
   - `src/renderer/hooks/agent/useModelProviderList.ts:30` — `ipcBridge.mode.getModelConfig.invoke()`
   - `src/renderer/pages/conversation/platforms/pancli/usePanCliModelSelection.ts:34-40` — 过滤 Google Auth

**影响范围**:

- Gate 2 用例设计: designer 需确认 TC-A-04（guid 页选模型）和 TC-A-07（对话中切换）的前置条件
- Gate 3 实现: engineer 需实现 `getPanCliTestModels()` helper（动态查询 provider 列表）
...
| `src/renderer/pages/conversation/platforms/pancli/PanCliChat.tsx`             | 19-57  | 对话页容器 + MessageList + SendBox                                  |
| `src/renderer/pages/conversation/platforms/pancli/PanCliSendBox.tsx`          | 88-459 | 发送框逻辑 + 文件附件 + 权限选择                                    |
| `src/renderer/pages/conversation/platforms/pancli/PanCliModelSelector.tsx`    | 19-135 | 模型选择器（对话页）                                                |
| `src/renderer/pages/conversation/platforms/pancli/usePanCliModelSelection.ts` | 24-73  | 模型选择 hook（过滤 google auth）                                   |
| `src/renderer/pages/conversation/platforms/pancli/usePanCliMessage.ts`        | 20-321 | 流式消息处理 + 工具状态                                             |
| `src/renderer/utils/model/agentModes.ts`                                      | 65-69  | aionrs 权限模式枚举                                                 |
| `src/process/task/PanCliManager.ts`                                           | 78-781 | 进程管理 + 权限审批 + DB 持久化                                     |
...
| `aioncore panai.db`                                                           | —      | conversations + messages 由 backend 独占持久化                      |
