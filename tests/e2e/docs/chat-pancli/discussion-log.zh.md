# Pan CLI (aionrs) E2E 测试 - 讨论记录

**项目**: PanAI E2E Coverage
**子组**: chat-pancli
**日期**: 2026-04-22
...
- [x] chat-pancli-designer 审核需求，补充测试用例设计建议
- [ ] chat-pancli-engineer 审核需求，评估技术可行性
...
2. **对话页需新增 15+ testid**（否则无法定位元素）
   - **P0 优先级**（阻塞测试）: 5 个
     ```tsx
     [data-testid="pancli-sendbox"]
     [data-testid="pancli-model-selector"]
...
3. **DB 清理需补充 helper**
   - 文件位置: `tests/e2e/helpers/chatPanCli.ts`
   - 函数: `cleanupE2EPanCliConversations()`, `getPanCliMessages()` 等
...
| 4. CI 环境 binary 来源？            | 短期 **CI skip**，长期 DevOps 配置       | `test.skip(() => resolvePanCliBinary() === null, 'aionrs binary not found')`              |
...
**背景**: 当前存储在 `PanCliApprovalStore`（内存），进程重启后失效
...
- **短期**: CI 跳过 aionrs E2E（`test.skip(() => resolvePanCliBinary() === null, 'aionrs binary not found')`）
- **长期**: DevOps 配置预装 或 动态下载（需 team-lead 协调）

**team-lead 决策**: **选 C CI 跳过**（短期）

- **理由**: 长期 CI 策略延后，不阻塞本轮 E2E 交付
- **实施**: `test.skip(() => resolvePanCliBinary() === null, 'aionrs binary not found')`
...
- [ ] chat-pancli-engineer review 用例设计，评估实现工作量
...
- `PanCliManager.ts:766-772` 调用 `agent.denyTool(callId, reason)` 发送 `tool_deny` 命令
