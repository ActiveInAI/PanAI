# Pan CLI (aionrs) E2E 测试用例

**版本**: Gate 2 初稿
**作者**: chat-pancli-designer
**日期**: 2026-04-22
**状态**: 待审核
...
1. **aionrs binary 可用**：通过 `ipcBridge.fs.findPanCliBinary.invoke()` 验证，否则 skip 全部测试
...
```typescript
// tests/e2e/helpers/chatPanCli.ts
...
| 维度       | 可选值                     | 说明                                                                                               |
| ---------- | -------------------------- | -------------------------------------------------------------------------------------------------- |
| 关联文件夹 | 无 / 单 / 多               | `atPath` 数组（源码：`PanCliSendBox.tsx:331-337`）                                                 |
...
| 对话中操作 | 切换模型 / 切换权限 / 无   | 对话页 `PanCliModelSelector` / `AgentModeSelector`                                                 |
...
- 同 §1.2 全局前置条件
- 调用 `getPanCliTestModels(page)` 获取 `modelA`（默认模型）
...
- 同 §1.2 全局前置条件
- 调用 `getPanCliTestModels(page)` 获取 `{ modelA, modelB }`
...
2. 等待首条 AI 回复完成
3. 点击对话页模型选择器（`PanCliModelSelector`，`data-testid="pancli-model-selector"`）
...
export async function checkPanCliBinary(): Promise<boolean> {
  try {
...
test.beforeAll(async () => {
  const hasBinary = await checkPanCliBinary();
  if (!hasBinary) {
...
1. **chat-pancli-engineer** review 本用例设计，评估实现工作量
...
| `src/renderer/pages/conversation/platforms/pancli/PanCliSendBox.tsx`          | 331-337  | `atPath` 状态（关联文件夹数组）                   |
| `src/renderer/pages/conversation/platforms/pancli/PanCliSendBox.tsx`          | 103-125  | 文件上传 handler                                  |
| `src/renderer/pages/conversation/platforms/pancli/PanCliSendBox.tsx`          | 206-212  | 发送消息（传递 `files` 参数）                     |
| `src/renderer/pages/conversation/platforms/pancli/usePanCliModelSelection.ts` | 36-40    | 过滤 Google Auth 模型                             |
| `src/renderer/utils/model/agentModes.ts`                                      | 65-69    | aionrs 权限枚举（`default`, `auto_edit`, `yolo`） |
| `src/process/task/PanCliManager.ts`                                           | 250-259  | 权限模式自动批准逻辑                              |
| `src/process/task/PanCliManager.ts`                                           | 727-737  | `setMode()` 持久化                                |
| `src/process/task/PanCliManager.ts`                                           | 452-489  | missing finish fallback（15s 超时）               |
