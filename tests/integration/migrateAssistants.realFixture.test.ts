 ...
 * (`/Users/zhoukai/Downloads/panai-config.txt` + `Archive/*.md`).
 *
 ...
 *
 *   1. Decode the legacy `panai-config.txt` exactly as ConfigStorage would.
...
const FIXTURE_CONFIG = '/Users/zhoukai/Downloads/panai-config.txt';
...
function resolveBackendBinary(): string {
  const candidates = [
    process.env.PANAI_BACKEND_BINARY,
    path.join(os.homedir(), '.cargo', 'bin', 'aioncore'),
    path.resolve(__dirname, '../../../PanAI backend/target/debug/aioncore'),
  ].filter((x): x is string => typeof x === 'string' && x.length > 0);
  for (const c of candidates) {
    if (existsSync(c)) return c;
  }
  throw new Error('aioncore binary not found (set PANAI_BACKEND_BINARY or build it)');
}
...
/**
 * Decode an `panai-config.txt` exactly the way `JsonFileBuilder` does:
...
describeIfFixtures('migrateAssistantsToBackend (real fixture)', () => {
  ...
  beforeEach(async () => {
    dataDir = await fsp.mkdtemp(path.join(os.tmpdir(), 'panai-migrate-fixture-'));
