...
if [ ! -d "$TEMP_DIR/panai-web" ]; then
  echo "❌ Missing panai-web directory"
  exit 1
fi

cd "$TEMP_DIR/panai-web"

# New layout (bun compile standalone binary):
#   panai-web/
#   ├── panai-web           ← single compiled executable (no bin/, no dist/, no node_modules)
...
if [ ! -x "panai-web" ]; then
  echo "❌ panai-web is not executable"
  exit 1
fi
echo "✓ panai-web is executable"

# 4. Test version command
echo ""
echo "4. Testing version command..."
VERSION=$(./panai-web version)
if [ -z "$VERSION" ]; then
...
HTTP_PORT=25899
DATA_DIR="$(mktemp -d)/panai-web-data"
...
./panai-web start --port "$HTTP_PORT" --data-dir "$DATA_DIR" > /tmp/panai-web.log 2>&1 &
SERVER_PID=$!
...
for i in $(seq 1 30); do
  if curl -sf "http://127.0.0.1:${HTTP_PORT}/" > /tmp/panai-web.html 2>/dev/null; then
    break
  fi
  sleep 1
done

if [ ! -s /tmp/panai-web.html ]; then
  kill "$SERVER_PID" 2>/dev/null || true
  wait "$SERVER_PID" 2>/dev/null || true
  echo "❌ HTTP probe failed — no response body. Server log:"
  cat /tmp/panai-web.log
  exit 1
fi

# Look for the SPA shell signature — <html + <div id="root" or similar marker
if grep -q '<html' /tmp/panai-web.html && grep -qE '<(div id="root"|script)' /tmp/panai-web.html; then
  echo "✓ HTTP root returns SPA index ($(wc -c < /tmp/panai-web.html) bytes)"
else
  kill "$SERVER_PID" 2>/dev/null || true
  wait "$SERVER_PID" 2>/dev/null || true
  echo "❌ HTTP root response does not look like SPA index:"
  head -20 /tmp/panai-web.html
  echo "---server log---"
  cat /tmp/panai-web.log
  exit 1
fi
...
if grep -q 'Backend binary not found' /tmp/panai-web.log; then
  ...
  for i in $(seq 1 20); do
    PASSWORD=$(grep -oE 'Generated initial admin password: [^ ]+' /tmp/panai-web.log | head -1 | sed 's/^Generated initial admin password: //')
    if [ -n "$PASSWORD" ]; then
      break
    fi
    sleep 1
