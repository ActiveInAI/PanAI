...
/**
 * Simplified build script for PanAI
...
try {
  ...
  if (process.platform === 'win32' && builderArgs.includes('--win')) {
    ...
    if (!cleaned) {
      const aionRunning = isProcessRunningWindows('PanAI.exe');
      const electronRunning = isProcessRunningWindows('electron.exe');
      if (aionRunning || electronRunning) {
        console.log('⚠️  Detected running PanAI/Electron process. Attempting to close...');
        killWindowsProcesses(['PanAI.exe', 'electron.exe']);
        cleaned = tryRemoveDir(winUnpackedDir);
        if (!cleaned) {
          console.log('⚠️  Directory still locked. Please close any running PanAI/Electron processes and retry.');
        }
      }
    }
  }
  ...
  try {
    ...
    const winExePath = path.join(outDir, 'win-unpacked', 'PanAI.exe');
    ...
    console.log('⚠️  Windows local build failed after PanAI.exe was produced.');
    if (firstError) {
