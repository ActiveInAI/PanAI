 ...
 * parsing the electron-log file for [PanAI:ready] / [PanAI:init] /
 * [PanAI:process] marks, plus `ready-to-show` / `did-finish-load` /
...
type StartupTiming = {
  ...
  // Parsed from [PanAI:init] marks
  initTotalMs: number;
  // Parsed from [PanAI:process] marks
...
function getLogFilePath(): string {
  ...
  if (process.platform === 'darwin') {
    candidates.push(
      path.join(os.homedir(), 'Library', 'Logs', 'PanAI-Dev', `${today}.log`),
      path.join(os.homedir(), 'Library', 'Logs', 'PanAI', `${today}.log`)
    );
  } else if (process.platform === 'win32') {
    const appData = process.env.APPDATA ?? path.join(os.homedir(), 'AppData', 'Roaming');
    candidates.push(path.join(appData, 'PanAI', 'logs', `${today}.log`));
  } else {
    candidates.push(path.join(os.homedir(), '.config', 'PanAI', 'logs', `${today}.log`));
  }
...
// Matches: [PanAI:ready] <label> +<ms>ms
// Matches: [PanAI:init]  <label> +<ms>ms
// Matches: [PanAI:process] <label> +<ms>ms
const MARK_REGEX = /\[PanAI:(ready|init|process)\]\s+([^+]+?)\s+\+(\d+)ms/;
...
function parseStartupLog(lines: string[]): ParsedMarks {
  ...
  for (const line of lines) {
    ...
    if (line.includes('[PanAI] Renderer did-finish-load')) marks.logs.rendererDidFinishLoad = true;
    else if (line.includes('[PanAI] Window ready-to-show')) marks.logs.windowReadyToShow = true;
    else if (line.includes('[PanAI] Showing main window')) marks.logs.showingMainWindow = true;
  }
