...
export INSTALL_DIR="/tmp/panai-web-smoke-test"
...
if [[ ! -x "${INSTALL_DIR}/panai-web" ]]; then
    echo "❌ CLI executable not found or not executable: ${INSTALL_DIR}/panai-web"
    exit 1
fi
echo "✓ CLI executable exists"

if [[ ! -L "${BIN_DIR}/panai-web" ]]; then
    echo "❌ Symlink not found: ${BIN_DIR}/panai-web"
    exit 1
fi
...
VERSION_OUTPUT=$(panai-web version 2>&1 || echo "")
