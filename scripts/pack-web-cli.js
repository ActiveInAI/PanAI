...
const tarballName = `panai-web-${version}-${normalizedPlatform}-${normalizedArch}.tar.gz`;
...
const tarballContentDir = path.join(stagingDir, 'panai-web');
...
const executableName = platform === 'win32' ? 'panai-web.exe' : 'panai-web';
...
// `panai-web version` match the tarball filename.
...
execSync(`tar -czf ${path.basename(tarballPath)} -C ${stagingDir} panai-web`, {
