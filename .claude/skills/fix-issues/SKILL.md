...
```bash
gh api repos/ActiveInAI/PanAI/issues/<number>/comments \
...
```bash
gh issue list --repo ActiveInAI/PanAI --state open --label bug --limit 50 \
...
gh api repos/ActiveInAI/PanAI/issues/<number>/timeline --jq \
...
```bash
gh issue comment <number> --repo ActiveInAI/PanAI --body "$(cat <<'EOF'
...
```bash
gh pr list --repo ActiveInAI/PanAI --state open --search "<error-keyword-or-file>" --json number,title
```
...
| --------- | ---------------- | ------------------------------------------------------------------- |
| repo      | ActiveInAI/PanAI | GitHub repository (owner/repo)                                      |
