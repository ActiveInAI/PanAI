...
# Check open PRs
gh pr list --repo ActiveInAI/PanAI --state open --search "<error-keyword>" --json number,title

# Check closed PRs (recently merged)
gh pr list --repo ActiveInAI/PanAI --state merged --search "<error-keyword>" --limit 5 --json number,title,mergedAt
```
