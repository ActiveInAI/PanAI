...
**サポートされている Agent：** 内蔵 Agent（ゼロ設定） • Claude Code • Codex • Qwen Code • Goose AI • OpenClaw • Augment Code • CodeBuddy • Kimi CLI • OpenCode • Factory Droid • GitHub Copilot • Qoder CLI • Mistral Vibe • Nanobot • Pan CLI（aionrs、PanAI に同梱される Rust バックエンドサービス） • Snow CLI • Hermes Agent • Cursor Agent など
...
- **並列マルチエージェント実行** — Leader がタスクをサブタスクに分解して並行実行する Teammate Agent に委任；各 Teammate は ACP（Agent Communication Protocol、PanAI のマルチエージェント調整層）、Gemini、または Aionrs を通じて独自のモデルを使用
...
- **対応バックエンド** — Claude Code、Codex、Gemini、Snow CLI、Pan CLI（aionrs）；`mcpCapabilities.stdio` を持つ他の ACP バックエンドも自動対応
