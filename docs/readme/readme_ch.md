...
**支持的 Agent：** 内置 Agent（零配置） • Claude Code • Codex • Qwen Code • Goose AI • OpenClaw • Augment Code • CodeBuddy • Kimi CLI • OpenCode • Factory Droid • GitHub Copilot • Qoder CLI • Mistral Vibe • Nanobot • Pan CLI（aionrs，PanAI 随附的 Rust 后端服务） • Snow CLI • Hermes Agent • Cursor Agent 等
...
- **多 Agent 并行执行** — Leader 将任务分解为子任务并委派给并行运行的 Teammate Agent；每个 Teammate 通过 ACP（Agent Communication Protocol，PanAI 的多 Agent 协调层）、Gemini 或 Aionrs 使用独立模型
...
- **支持的后端** — Claude Code、Codex、Gemini、Snow CLI、Pan CLI（aionrs）；其他具备 `mcpCapabilities.stdio` 的 ACP 后端自动支持
...
| AWS Bedrock 凭证                         | 通过 Pan CLI（aionrs）的 Bedrock 驱动 Agent |
