...
**支援的 Agent：** 內建 Agent（零配置） • Claude Code • Codex • Qwen Code • Goose AI • OpenClaw • Augment Code • CodeBuddy • Kimi CLI • OpenCode • Factory Droid • GitHub Copilot • Qoder CLI • Mistral Vibe • Nanobot • Pan CLI（aionrs，隨附於 PanAI 的 Rust 後端服務） • Snow CLI • Hermes Agent • Cursor Agent 等
...
- **多 Agent 並行執行** — Leader 將任務分解為子任務並委派給並行運行的 Teammate Agent；每個 Teammate 透過 ACP（Agent Communication Protocol，PanAI 的多 Agent 協調層）、Gemini 或 Aionrs 使用獨立模型
...
- **支援的後端** — Claude Code、Codex、Gemini、Snow CLI、Pan CLI（aionrs）；其他具備 `mcpCapabilities.stdio` 的 ACP 後端自動支援
...
| AWS Bedrock 憑證                         | 透過 Pan CLI（aionrs）的 Bedrock 驅動 Agent |
