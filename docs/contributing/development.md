...
git clone https://github.com/ActiveInAI/PanAI.git
cd PanAI
...
When you have two clones of the repository (e.g. `PanAI` and `PanAI-refactor`) and need to run both simultaneously, the second instance can be started with:
...
- Isolates data/config symlink paths (`~/.panai-dev-2`, `~/.panai-config-dev-2`)
...
PanAI uses **electron-vite** for fast bundling:
