# PanAI WebUI Mode - Startup Guide

PanAI supports WebUI mode, allowing you to access the application through a web browser. This guide covers how to start WebUI mode on all supported platforms.
...
WebUI mode starts PanAI with an embedded web server, allowing you to:

- Access the application through any modern web browser
- Use PanAI from remote devices on the same network (with `--remote` flag)
...
```cmd
# Using full path
"C:\Program Files\PanAI\PanAI.exe" --webui

# Or if PanAI is in your PATH
PanAI.exe --webui
```
...
```bash
# Using full path
/Applications/PanAI.app/Contents/MacOS/PanAI --webui

# Or using open command
open -a PanAI --args --webui
```

### Method 2: Create Shell Script

Create `start-panai-webui.sh`:
...
```bash
chmod +x start-panai-webui.sh
./start-panai-webui.sh
```
...
chmod +x PanAI-*.AppImage

# Run with --webui flag
./PanAI-*.AppImage --webui
```

### Method 2: Create Desktop Entry

Create `~/.local/share/applications/panai-webui.desktop`:

```ini
[Desktop Entry]
Name=PanAI WebUI
Comment=Start PanAI in WebUI mode
Exec=/opt/PanAI/PanAI --webui
Icon=panai
Terminal=false
Type=Application
...
```bash
chmod +x ~/.local/share/applications/panai-webui.desktop
```
...
Create `~/bin/start-panai-webui.sh`:
...
```bash
chmod +x ~/bin/start-panai-webui.sh
```
...
Create `/etc/systemd/system/panai-webui.service`:

```ini
[Unit]
Description=PanAI WebUI Service
After=network.target

[Service]
Type=simple
User=YOUR_USERNAME
ExecStart=/opt/PanAI/PanAI --webui --remote
Restart=on-failure
RestartSec=10
...
sudo systemctl enable panai-webui.service
sudo systemctl start panai-webui.service

# Check status
sudo systemctl status panai-webui.service
```
...
**Important Note**: Electron desktop mode is **not supported** on Android. However, you can run PanAI in WebUI mode using Termux with a prooted Linux environment.

> **Community Contribution**: This guide is contributed by [@Manamama](https://github.com/Manamama). Special thanks for making PanAI accessible on Android devices! 🙏
>
> **Original Tutorial**: [Running PanAI WebUI on Android via Termux + Proot Ubuntu](https://gist.github.com/Manamama/b4f903c279b5e73bdad4c2c0a58d5ddd)
>
> **Related Issues**: [#217 - Android Support Discussion](https://github.com/ActiveInAI/PanAI/issues/217)
...
#### 4. Download and Install PanAI

```bash
# Download the ARM64 .deb package (replace VERSION with the actual version)
# Check latest version at: https://github.com/ActiveInAI/PanAI/releases
wget https://github.com/ActiveInAI/PanAI/releases/download/vVERSION/PanAI_VERSION_arm64.deb

# Example (replace VERSION with the release tag, e.g. v1.5.2):
wget https://github.com/ActiveInAI/PanAI/releases/download/vVERSION/PanAI_VERSION_arm64.deb

# Install the package
apt install -y ./PanAI_*.deb

# Verify installation
which PanAI
```

#### 5. Launch PanAI WebUI

```bash
# Start PanAI in WebUI mode with no-sandbox flag
PanAI --no-sandbox --webui
```
...
To access PanAI from other devices on your local network:

```bash
# Start with --remote flag
PanAI --no-sandbox --webui --remote
...
PanAI --no-sandbox --webui --port 8080
```
...
chmod +x /opt/PanAI/PanAI
```

#### Out of Memory

PanAI requires sufficient RAM. Close other apps if you encounter memory issues.

#### Cannot Access from Browser

1. Check if PanAI is running: look for "Server started" message
...
4. **Keep device charged** - Running PanAI consumes battery

### Tested Environment

- **Device**: Android 14
- **Termux Version**: 0.118.0
- **PanAI Version**: Latest release (e.g. 1.5.2)
- **Proot-distro**: Ubuntu (latest)

### Creating a Startup Script

For convenience, create a script to launch PanAI quickly:

```bash
# Create script in Ubuntu (proot)
cat > ~/start-panai.sh << 'EOF'
#!/bin/bash
echo "Starting PanAI WebUI..."
PanAI --no-sandbox --webui --remote
EOF

# Make executable
chmod +x ~/start-panai.sh
...
```bash
proot-distro login ubuntu -- bash -c "PanAI --no-sandbox --webui --remote"
```
...
2. Report issues at [GitHub Issues #217](https://github.com/ActiveInAI/PanAI/issues/217)
...
```cmd
PanAI.exe --webui --remote
```

### macOS

```bash
/Applications/PanAI.app/Contents/MacOS/PanAI --webui --remote
```
...
netsh advfirewall firewall add rule name="PanAI WebUI" dir=in action=allow protocol=TCP localport=3000
```
...
**macOS:**
Go to **System Preferences** → **Security & Privacy** → **Firewall** → **Firewall Options** → Add PanAI
...
which PanAI
# or
find /opt -name "PanAI" 2>/dev/null
```
...
```powershell
& "C:\Program Files\PanAI\PanAI.exe" --webui 2>&1 | Tee-Object -FilePath panai.log
```

**macOS/Linux:**

```bash
/path/to/PanAI --webui 2>&1 | tee panai.log
```
...
export PANAI_PORT=8080

# Allow remote access without passing --remote
export PANAI_ALLOW_REMOTE=true

# Optional host hint (0.0.0.0 behaves the same as PANAI_ALLOW_REMOTE=true)
export PANAI_HOST=0.0.0.0

# Then start the application
PanAI --webui

# You can also pass the port directly via CLI
PanAI --webui --port 8080
```
...
| Platform | Location                                                 |
| -------- | -------------------------------------------------------- |
| Windows  | `%APPDATA%/PanAI/webui.config.json`                     |
| macOS    | `~/Library/Application Support/PanAI/webui.config.json` |
| Linux    | `~/.config/PanAI/webui.config.json`                     |
...
```cmd
# Using full path
"C:\Program Files\PanAI\PanAI.exe" --resetpass

# Or for a specific user
"C:\Program Files\PanAI\PanAI.exe" --resetpass username
```
...
```bash
# Using full path
/Applications/PanAI.app/Contents/MacOS/PanAI --resetpass

# Or for a specific user
/Applications/PanAI.app/Contents/MacOS/PanAI --resetpass username
```
...
# Or for a specific user
PanAI --resetpass username

# Or using full path
/opt/PanAI/PanAI --resetpass
```
...
- [GitHub Issues](https://github.com/ActiveInAI/PanAI/issues)
...
2. Search [existing issues](https://github.com/ActiveInAI/PanAI/issues)
3. Create a [new issue](https://github.com/ActiveInAI/PanAI/issues/new) with:
   - Your OS and version
   - PanAI version
   - Steps to reproduce
...
**Happy using PanAI in WebUI mode!** 🚀
