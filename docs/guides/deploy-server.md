# PanAI Headless Server Deployment Guide

Deploy PanAI WebUI on headless Linux servers — cloud VMs, Kubernetes Pods, and containers — with proxy auto-fallback support.
...
- At least 2GB RAM
- PanAI `.deb` package from [Releases](https://github.com/ActiveInAI/PanAI/releases)
...
wget https://github.com/ActiveInAI/PanAI/releases/latest/download/PanAI-linux-amd64.deb

# Install
sudo dpkg -i PanAI-linux-amd64.deb
...
PanAI is an Electron app and requires a display server. On headless servers (no monitor), use Xvfb to create a virtual display:
...
Create `/opt/PanAI/start-panai.sh`:

```bash
#!/bin/bash
# PanAI WebUI headless startup script
# Usage: ./start-panai.sh [start|stop|restart|status]
...
start() {
    if [ -f "$PIDFILE" ] && kill -0 "$(cat $PIDFILE)" 2>/dev/null; then
        echo "PanAI is already running (PID: $(cat $PIDFILE))"
        return 1
    fi
    ...
    if kill -0 "$(cat $PIDFILE)" 2>/dev/null; then
        echo "PanAI started successfully (PID: $(cat $PIDFILE))"
        echo "WebUI: http://$(hostname -I | awk '{print $1}'):25808"
    else
        echo "PanAI failed to start. Check log: $LOGFILE"
        rm -f "$PIDFILE"
        return 1
    fi
}
...
stop() {
    if [ ! -f "$PIDFILE" ]; then
        echo "PanAI is not running (no PID file)"
        return 1
    fi
    PID=$(cat "$PIDFILE")
    echo "Stopping PanAI (PID: $PID)..."
    kill "$PID" 2>/dev/null
    sleep 2
...
status() {
    if [ -f "$PIDFILE" ] && kill -0 "$(cat $PIDFILE)" 2>/dev/null; then
        echo "PanAI is running (PID: $(cat $PIDFILE))"
        ss -tlnp | grep 25808
    else
        echo "PanAI is not running."
...
```bash
chmod +x /opt/PanAI/start-panai.sh
```

> **Tip**: `WORKDIR` determines the directory PanAI can access for file operations. Set it to your project workspace.

---

## Remote Access

PanAI WebUI listens on port **25808**. Choose a method based on your network setup:
...
### Step 2: PAC File for PanAI (Electron / Chromium Layer)
...
```bash
    nohup xvfb-run --auto-servernum --server-args="-screen 0 1920x1080x24" \
        /usr/bin/PanAI --webui --remote --no-sandbox \
        --proxy-pac-url="file:///opt/PanAI/proxy.pac" \
        > "$LOGFILE" 2>&1 &
```
...
### Step 4: PanAI Internal Proxy (Gemini API)

For Gemini API calls, configure the proxy inside PanAI WebUI:

**Settings → Gemini Settings → Proxy** → `http://127.0.0.1:7897`

> This proxy is handled by PanAI's Node.js layer (separate from the Chromium layer). When the SSH tunnel is down, Gemini API calls will fail, but the WebUI and other APIs remain functional.
...
| Issue                                     | Solution                                                     |
| ----------------------------------------- | ------------------------------------------------------------ |
| `dpkg` dependency errors in containers    | `dpkg --force-all -i PanAI-linux-amd64.deb`                 |
| PanAI can only access `/tmp`             | Set `WORKDIR` in the startup script to your workspace path   |
...
在无图形界面的 Linux 服务器（云主机、K8s Pod、容器）上部署 PanAI WebUI，支持代理自动回退。

## 前置条件

- Linux x86_64（推荐 Ubuntu 20.04+ / Debian 11+）
- 至少 2GB 内存
- PanAI `.deb` 安装包（[下载地址](https://github.com/ActiveInAI/PanAI/releases)）
...
PanAI 是 Electron 应用，需要显示服务。无头服务器需安装 Xvfb：
...
创建 `/opt/PanAI/start-panai.sh`：

```bash
#!/bin/bash
# PanAI WebUI 无头启动脚本
# 用法: ./start-panai.sh [start|stop|restart|status]
...
start() {
    if [ -f "$PIDFILE" ] && kill -0 "$(cat $PIDFILE)" 2>/dev/null; then
        echo "PanAI 已在运行 (PID: $(cat $PIDFILE))"
        return 1
    fi
    echo "正在启动 PanAI WebUI..."
    cd "$WORKDIR"
    ...
    sleep 3
    if kill -0 "$(cat $PIDFILE)" 2>/dev/null; then
        echo "PanAI 启动成功 (PID: $(cat $PIDFILE))"
        echo "WebUI: http://$(hostname -I | awk '{print $1}'):25808"
    else
        echo "PanAI 启动失败，请查看日志: $LOGFILE"
        rm -f "$PIDFILE"
        return 1
    fi
}
...
status() {
    if [ -f "$PIDFILE" ] && kill -0 "$(cat $PIDFILE)" 2>/dev/null; then
        echo "PanAI 运行中 (PID: $(cat $PIDFILE))"
        ss -tlnp | grep 25808
    else
        echo "PanAI 未在运行。"
        rm -f "$PIDFILE" 2>/dev/null
...
PanAI WebUI 监听端口 **25808**，根据网络环境选择访问方式：
...
### 第二步：PAC 代理文件（PanAI Electron 层）

`--proxy-server` 的问题：代理一断，**所有请求**全挂。改用 PAC 文件实现自动回退。

创建 `/opt/PanAI/proxy.pac`：
...
启动脚本中添加参数：`--proxy-pac-url="file:///opt/PanAI/proxy.pac"`
...
| PanAI 只能访问 /tmp   | 修改启动脚本中的 `WORKDIR`            |
